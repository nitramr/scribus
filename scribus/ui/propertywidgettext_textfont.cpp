...
void PropertyWidgetText_TextFont::setMainWindow(ScribusMainWindow* mw)
{
	m_ScMW = mw;
...
void PropertyWidgetText_TextFont::setDoc(ScribusDoc *d)
{
...
void PropertyWidgetText_TextFont::unsetDoc()
{
	if (m_doc)
	{
		disconnect(m_doc->m_Selection, SIGNAL(selectionChanged()), this, SLOT(handleSelectionChanged()));
...
void PropertyWidgetText_TextFont::setCurrentItem(PageItem *i)
{
...
PageItem* PropertyWidgetText_TextFont::currentItemFromSelection()
{
...
void PropertyWidgetText_TextFont::handleLineSpacingMode(int id)
{
...
void PropertyWidgetText_TextFont::showLineSpacing(double r)
{
	if (!m_ScMW || m_ScMW->scriptIsRunning())
		return;
...
void PropertyWidgetText_TextFont::setupLineSpacingSpinbox(int mode, double value)
{
...
void PropertyWidgetText_TextFont::handleLineSpacing()
{
...
void PropertyWidgetText_TextFont::showFontFace(const QString& newFont)
{
	if (!m_ScMW || m_ScMW->scriptIsRunning())
		return;
	bool tmp = m_haveItem;
...
void PropertyWidgetText_TextFont::showFontSize(double s)
{
	if (!m_ScMW || m_ScMW->scriptIsRunning())
		return;
...
void PropertyWidgetText_TextFont::showTextColors(QString b, double shb)
{
...
void PropertyWidgetText_TextFont::handleFontSize()
{
	if (!m_haveDoc || !m_haveItem || !m_ScMW || m_ScMW->scriptIsRunning())
		return;
	Selection tempSelection(this, false);
...
void PropertyWidgetText_TextFont::handleTextFont(QString c)
{
	if (!m_haveDoc || !m_haveItem || !m_ScMW || m_ScMW->scriptIsRunning())
		return;
	m_ScMW->SetNewFont(c);
...
// Fills
void PropertyWidgetText_TextFont::handleFillColorBox(){
...
// Fills
void PropertyWidgetText_TextFont::handleTextFill()
{
	if (!m_doc || !m_item || !m_ScMW || m_ScMW->scriptIsRunning())
		return;
	PageItem *i2 = m_item;
...
void PropertyWidgetText_TextFont::handleTextShade()
{
	if (!m_doc || !m_item || !m_ScMW || m_ScMW->scriptIsRunning())
		return;
...
void PropertyWidgetText_TextFont::updateColorList()
{
...
void PropertyWidgetText_TextFont::updateCharStyle(const CharStyle& charStyle)
{
	if (!m_ScMW || m_ScMW->scriptIsRunning())
		return;
...
void PropertyWidgetText_TextFont::updateStyle(const ParagraphStyle& newCurrent)
{
	if (!m_ScMW || m_ScMW->scriptIsRunning())
		return;
...
void PropertyWidgetText_TextFont::handleUpdateRequest(int updateFlags)
{
...
void PropertyWidgetText_TextFont::handleSelectionChanged()
{
...
void PropertyWidgetText_TextFont::languageChange()
{
	retranslateUi(this);
...
void PropertyWidgetText_TextFont::changeEvent(QEvent *e)
{
